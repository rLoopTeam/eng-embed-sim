...
class Sim(object):
    ...
    def step(self, dt_usec):        
        ...
        if self.n_steps_taken % 500 == 0:
            ...
            self.pda_list.append(self.pod.acceleration)
            self.pdv_list.append(self.pod.velocity)
            self.pdp_list.append(self.pod.position)
    ...
    def run(self):
        ...
        ax1.plot(self.pdp_list, self.pdv_list, 'b')

        ax2 = ax1.twinx()
        ax2.set_ylabel('acceleration (m/s^2)', color='r')
        ax2.tick_params('y', colors='r')
        ax2.plot(self.pdp_list, self.pda_list, 'r')
