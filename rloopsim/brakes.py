...
class Brakes(object):
    ...
    def apply(self):
        # Apply the brakes (move to min gap from current pos)
        for brake in self._list:
            brake.apply()

    def retract(self):
        # Retract the brakes to their outermost position
        for brake in self._list:
            brake.retract()

    def hold(self):
        # Keep the brakes where they are
        for brake in self._list:
            brake.hold()
...
class Brake:
    ...
    def _move_to_gap_target(self, gap_target):
        # TESTING ONLY -- if the move the brakes stanza is uncommented below, this will work
        self._gap_target = gap_target
    
    def apply(self):
        # Apply the brakes (move to min gap from current pos)
        self._move_to_gap_target(self.extended_gap)

    def retract(self):
        # Retract the brakes to their outermost position
        self._move_to_gap_target(self.retracted_gap)
